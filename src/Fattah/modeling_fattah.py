# coding=utf-8
# Copyright 2024 fattah AI and the HuggingFace Inc. team. All rights reserved.
#
...
from .configuration_fattah import fattahConfig
...
_CHECKPOINT_FOR_DOC = "fattah"
_CONFIG_FOR_DOC = "fattahConfig"
...
# Copied from transformers.models.llama.modeling_llama.LlamaRMSNorm with Llama->fattah
class fattahRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        fattahRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class fattahRotaryEmbedding(nn.Module):
...
class fattahParallelExperts(nn.Module):
    def __init__(self, num_experts: int, input_size: int, output_size: int) -> None:
        """
        Initialize the fattahParallelExperts module.
    ...
    def forward(self, inputs, expert_size):
        """
        Forward pass of the fattahParallelExperts module.
...
class fattahTopKGating(nn.Module):
...
class fattahMoE(nn.Module):
    ...
    def __init__(self, config: fattahConfig):
        super(fattahMoE, self).__init__()

        self.input_size = config.hidden_size
        self.hidden_size = config.intermediate_size
        self.activation = ACT2FN[config.activation_function]
        self.bias = torch.nn.Parameter(torch.empty(self.input_size))
        self.input_linear = fattahParallelExperts(config.num_local_experts, self.input_size, self.hidden_size * 2)
        self.output_linear = fattahParallelExperts(config.num_local_experts, self.hidden_size, self.input_size)

        self.router = fattahTopKGating(
...
class fattahMoA(nn.Module):
    ...
    def __init__(self, config: fattahConfig):
        ...
        self.input_linear = fattahParallelExperts(self.num_experts, self.input_size, self.hidden_size)
        self.output_linear = fattahParallelExperts(self.num_experts, self.hidden_size, self.input_size)
...
class fattahAttention(nn.Module):
    """
    Multi-headed attention from 'Attention Is All You Need' paper.
    """

    def __init__(self, config: fattahConfig, layer_idx: Optional[int] = None):
        """
        Initialize the fattahAttention module.
        ...
        self.experts = fattahMoA(config)

        self.kv_proj = torch.nn.Linear(self.hidden_size, self.kv_projection_size * 2, bias=False)

        self.rotary_emb = fattahRotaryEmbedding(
            config.kv_channels,
...
class fattahSdpaAttention(fattahAttention):
    """
    fattah attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `fattahAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from fattahAttention.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "fattahModel is using fattahSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
class fattahFlashAttention2(fattahAttention):
    ...
    def forward(
        ...
        """
        Forward pass of the fattahAttention module.
...
fattah_ATTENTION_CLASSES = {
    "eager": fattahAttention,
    "flash_attention_2": fattahFlashAttention2,
    "sdpa": fattahSdpaAttention,
}


class fattahBlock(nn.Module):
    def __init__(self, config: fattahConfig, layer_idx: Optional[int] = None):
        """
        Initialize the fattahBlock module.
        ...
        """
        super().__init__()
        self.input_layernorm = fattahRMSNorm(config.hidden_size)
        self.self_attention = fattah_ATTENTION_CLASSES[config._attn_implementation](config, layer_idx)
        self.post_attention_layernorm = fattahRMSNorm(config.hidden_size)

        self.mlp = fattahMoE(config)
...
class fattahPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = fattahConfig
    base_model_prefix = "transformer"
    supports_gradient_checkpointing = False
    _no_split_modules = ["fattahBlock"]
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, fattahParallelExperts):
            module.weight.data.normal_(mean=0.0, std=self.config.initializer_range)
        elif isinstance(module, fattahMoA):
            module.bias.data.zero_()
        elif isinstance(module, fattahMoE):
            module.bias.data.zero_()


fattah_START_DOCSTRING = r"""
    This model is a PyTorch [torch.nn.Module](https://pytorch.org/docs/stable/nn.html#torch.nn.Module) sub-class. Use
    it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage and
    behavior.

    Parameters:
        config ([`fattahConfig`]): Model configuration class with all the parameters of the model.
            Initializing with a config file does not load the weights associated with the model, only the
            configuration. Check out the [`~PreTrainedModel.from_pretrained`] method to load the model weights.
"""

fattah_INPUTS_DOCSTRING = r"""
    Args:
...
@add_start_docstrings(
    "The bare fattah Model outputting raw hidden-states without any specific head on top.",
    fattah_START_DOCSTRING,
)
class fattahModel(fattahPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`fattahBlock`]
    ...
    def __init__(self, config: fattahConfig):
        ...
        self.layers = nn.ModuleList([fattahBlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
        self._attn_implementation = config._attn_implementation
        self.norm = fattahRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    @add_start_docstrings_to_model_forward(fattah_INPUTS_DOCSTRING)
    def forward(
        self,
        ...
        if attention_mask is not None and self._attn_implementation == "flash_attention_2" and use_cache:
            ...
            if is_padding_right:
                raise ValueError(
                    ...
                    " this may lead to unexpected behaviour for Flash Attention version of fattah. Make sure to "
...
class fattahForCausalLM(fattahPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = fattahModel(config)
        self.vocab_size = config.vocab_size
...
@add_start_docstrings(
    """
    The fattah Model transformer with a sequence classification head on top (linear layer).

    [`fattahForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-2) do.
...
# Copied from transformers.models.llama.modeling_llama.LlamaForSequenceClassification with Llama->fattah, LLAMA->fattah
class fattahForSequenceClassification(fattahPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
